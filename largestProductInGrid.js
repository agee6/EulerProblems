...
var largeSum = function(numberArr){
  ...
  for (var i = 1; i < numberArr.length-1; i++) {
...
// var records = getRecordsFromFile("triangleGrid.txt", largeSum);
...
var createArray = function(){
  ...
  var gridSpit = grid.split(' ');
  var finalGrid = [];
  var row = [];
  for (var i = 0; i < gridSpit.length; i++) {
    row.push(parseInt(gridSpit[i]));
    if(row.length % 20 === 0){
      finalGrid.push(row);
      row = [];
    }
  }
  // findLargestProduct(4, finalGrid);
};

// var findLargestProduct =  function(num, grid){
//   var maxProduct = 0;
...
//       }
//       currentProduct = 1;
//       if((i + num <= grid.length) && (j + num <= grid[i].length)){
//         for (var k = 0; k < num; k++) {
...
//       }
//       currentProduct = 1;
//       if((i - num >= -1) &&(j + num <= grid[i].length)){
//         for (var k = 0; k < num; k++) {
//           currentProduct = currentProduct * (grid[i - k][j + k]);
//         }
//         if(currentProduct > maxProduct){
...
var maximumPathSum = function(grid){
  grid[0][0] = parseInt(grid[0][0]);
  var currentMax = 0;
  var temp1, temp2;
  var i = 1;
  while(i < grid.length){
    for (var j = 0; j < grid[i].length; j++) {
      if(j === 0){
        grid[i][j] = parseInt(grid[i][j]) + grid[i-1][j];
      }else if(j === grid[i].length - 1){
        grid[i][j] = parseInt(grid[i][j]) + grid[i-1][j-1];
      }else{
        temp1 = parseInt(grid[i][j]) + grid[i-1][j-1];
        temp2 = parseInt(grid[i][j]) + grid[i-1][j];
        if(temp1 > temp2){
          grid[i][j] = temp1;
        }else{
          grid[i][j] = temp2;
...
//var records = getRecordsFromFile("grid.txt", findLargestProduct);
